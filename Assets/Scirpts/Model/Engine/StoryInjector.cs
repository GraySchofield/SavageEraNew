...
[Serializable]
public class StoryInjector: IRecorderObserver
{
	// Require -> Subjects
	...
	public void Notify(Recorder recorder, string type){
		...
		if (count == 1) {
			UnlockStoriesWithRequire (type, recorder);
		}
	}
	...
	private void UnlockStoriesWithRequire(string require, Recorder recorder){
		if (require_to_subjects.ContainsKey (require)) {
			...
			foreach (string potential_unlockable in potential_unlockables) {
				if (IsUnlockable (potential_unlockable, recorder)) {
					Game.Current.EventEngine.AddEvent (Config.SpawnEventTriggerType, EventFactory.Get (potential_unlockable));
				}
			}
		}
	}
	
	private void UnlockStory(string type, Recorder recorder){
		if (IsUnlockable (type, recorder)) {
			Game.Current.EventEngine.AddEvent (Config.SpawnEventTriggerType, EventFactory.Get (type));
		}
	}
	...
	public void Run(){	
		...
		XmlNodeList actionNodes = actionSettings.SelectNodes ("/Events/Event[@class='spawn_story_event']");
		foreach (XmlNode actionNode in actionNodes) {
            ...
            if (noRequire && g.Recorder.Get(type) == null)
            {
                Game.Current.EventEngine.AddEvent(Config.SpawnEventTriggerType, EventFactory.Get(type));
            }
          }
		...
		XmlNodeList action_subjects = actionSettings.SelectNodes ("/Events/Event[Require and @class='spawn_story_event']");
	...
	private void UpdateRequires(XmlNodeList nodeList, XmlDocument itemSettings){
		foreach (XmlNode node in nodeList) {
			...
			if (Game.Current.Recorder.Get(subject) == null && subject_to_requires.ContainsKey(subject)){
				UnlockStory(subject, Game.Current.Recorder);
			}
		}
	}
